...
export const generateMessageShareUrl = async (req, res) => {
  try {
    ...
    const shareToken = message.generateShareToken();
    await message.save();

    // Generate shareable URL
    const shareUrl = `https://farmcare-backend-new.onrender.com/api/v1/message/share/${shareToken}`;

    res.status(200).json({
      success: true,
      shareUrl,
      expiresAt: message.shareTokenExpiry
    });
  } catch (error) {
    console.error('Generate message share URL error:', error);
    res.status(500).json({
      success: false,
...
export const getSharedMessage = async (req, res) => {
  try {
    const { shareToken } = req.params;
    const message = await ChannelMessage.findOne({ shareToken })
      .populate('channelId', 'name'); // Get channel name

    if (!message || !message.isShareTokenValid()) {
      return res.status(404).json({
        success: false,
    ...
    res.status(500).json({
      success: false,
      message: 'Error retrieving shared message'
    });
  }
};