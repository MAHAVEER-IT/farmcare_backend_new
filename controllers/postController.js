...
export const generateShareUrl = async (req, res) => {
  try {
    const { postId } = req.params;
    const post = await Post.findOne({ postId });
    ...
    const shareToken = post.generateShareToken();
    await post.save();

    // Generate shareable URL
    const shareUrl = `https://farmcare-backend-new.onrender.com/api/v1/posts/share/${shareToken}`;

    res.status(200).json({
      success: true,
      shareUrl,
      expiresAt: post.shareTokenExpiry
    });
  } catch (error) {
...
export const getSharedPost = async (req, res) => {
  try {
    ...
    const post = await Post.findOne({ shareToken });

    if (!post || !post.isShareTokenValid()) {
      return res.status(404).json({
        success: false,
    ...
    res.status(500).json({
      success: false,
      message: 'Error retrieving shared post'
    });
  }
};
